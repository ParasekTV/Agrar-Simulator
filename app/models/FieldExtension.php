...
trait FieldExtension
{
    ...
    /**
     * Berechnet die nächste freie Position für ein neues Feld
     */
    private function getNextFieldPosition(int $farmId): array
    {
        $maxPos = $this->db->fetchOne(
            'SELECT MAX(position_x) as max_x, MAX(position_y) as max_y FROM fields WHERE farm_id = ?',
            [$farmId]
        );
        ...
        // Neue Reihe starten wenn x > 10
        if ($x > 10) {
            $x = 1;
            $y = $y + 1;
        }

        return ['x' => $x, 'y' => max(1, $y)];
    }
    ...
    public function buyMeadow(int $farmId, float $size): array
    {
        ...
        if (!$limit) {
            return ['success' => false, 'message' => 'Ungültige Wiesengröße'];
        }
        ...
        $pos = $this->getNextFieldPosition($farmId);
    ...
    public function buyGreenhouse(int $farmId, float $size): array
    {
        ...
        if (!$limit) {
            return ['success' => false, 'message' => 'Ungültige Gewächshausgröße'];
        }
    ...
    /**
     * Gibt Gewächshaus-Pflanzen zurück (nur freigeschaltete)
     */
    public function getGreenhouseCrops(int $farmId): array
    {
        $farm = new Farm($farmId);
        ...
        return $this->db->fetchAll(
            "SELECT * FROM crops
             WHERE is_greenhouse_only = TRUE
               AND (required_research_id IS NULL OR required_research_id IN ({$placeholders}))
    ...
    /**
     * Gibt alle Gewächshaus-Pflanzen zurück (für Info-Anzeige)
     */
    public function getAllGreenhouseCrops(): array
    {
        return $this->db->fetchAll(
            "SELECT c.*, rt.name as research_name
             FROM crops c
             LEFT JOIN research_tree rt ON c.required_research_id = rt.id
             WHERE c.is_greenhouse_only = TRUE
